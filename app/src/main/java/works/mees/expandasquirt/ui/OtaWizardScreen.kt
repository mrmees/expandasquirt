...
import works.mees.expandasquirt.ota.EXPANDASQUIRT_R4_MAX_SKETCH_BYTES
import works.mees.expandasquirt.ota.SizeCheck
import works.mees.expandasquirt.ota.rememberBinFilePicker
    ...
    Scaffold(
        ...
        Column(
            ...
            when (val currentStep = step) {
                ...
                OtaStep.Applying -> BusyStep("Expandasquirt is flashing the firmware. Don't disturb.")
...
private fun PickFileStep(otaVm: OtaViewModel) {
    ...
    Column(
        ...
        Text("Pick a `.bin` to flash to the Expandasquirt. Engine off; stable power.")
...
private fun PreFlightStep(
    ...
    Column(
        ...
        when (val check = step.sizeCheck) {
            SizeCheck.Ok -> Text(
                "Size OK (${step.sizeBytes} / $EXPANDASQUIRT_R4_MAX_SKETCH_BYTES bytes)",
