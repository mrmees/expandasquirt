...
import works.mees.expandasquirt.ble.DumpFrame
import works.mees.expandasquirt.ble.DumpParser
...
class DashboardViewModel(
    ...
    init {
        ...
        viewModelScope.launch {
            ble.lines.collect { line ->
                ...
                if (frame != null) {
                    frame.readings.forEach { (name, reading) ->
                        val history = histories.getOrPut(name) { ArrayDeque() }
                        history.addLast(reading.value.toFloat())
                    ...
                    _state.update {
                        it.copy(
                            frame = frame,
                            histories = histories.mapValues { entry -> entry.value.toList() },
                            lastFrameMonoNs = System.nanoTime(),
                        )
                    }
                }
            }
    ...
    private companion object {
        const val HISTORY_CAPACITY = 300
    }
}
