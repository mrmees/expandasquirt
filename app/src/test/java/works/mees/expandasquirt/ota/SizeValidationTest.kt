...
class SizeValidationTest {
    ...
    @Test fun acceptsAtMaxBoundary() {
        assertEquals(SizeCheck.Ok, validateSketchSize(EXPANDASQUIRT_R4_MAX_SKETCH_BYTES.toLong()))
    }

    @Test fun rejectsOneByteOverMax() {
        val r = validateSketchSize(EXPANDASQUIRT_R4_MAX_SKETCH_BYTES + 1L) as SizeCheck.TooLarge
        assertEquals(EXPANDASQUIRT_R4_MAX_SKETCH_BYTES + 1L, r.bytes)
        assertEquals(EXPANDASQUIRT_R4_MAX_SKETCH_BYTES, r.max)
    }
